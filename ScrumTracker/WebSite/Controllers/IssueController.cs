...
namespace WebSite.Controllers
{
    ...
    public class IssueController : BaseController
    {
        ...
        public ActionResult Index(Guid? id, Guid? issueId, Guid? sprintId, string stateName)
        {
            ...
            //var dtos = projects as ProjectDto[] ?? projects.ToArray();
        ...
        public ActionResult DeleteIssue(DeleteIssueModel model)
        {
            ...
            if (issue != null)
            {
                //if (issue.TimeTrackings != null)
                //{
                //    _issueManager.RemoveTimeTracking(issue.TimeTrackings);
                //    issue.TimeTrackings = null;
                //}
        ...
        public ActionResult SearchIssues(string term, int? page, string projectId, string sortFilter, Guid? issueId)
        {
            ...
            //{
            //    case "open":
            //        issues = issues.Where(x => x.State == _issueManager.GetStateByName("Open"));
            //        break;
            //    case "my":
            //        issues = issues.Where(x => x.AssigneeId == CurrentUserId);
            //        break;
            //    case "in-progress":
            //        issues = issues.Where(x => x.State == _issueManager.GetStateByName("In Progress"));
            //        break;
            //}
        ...
        public ActionResult AutocompleteIssueSearch(string term)
        {
            var model = _issueManager.GetAllIssues().Where(x => x.Name.Contains(term)).OrderBy(x => x.Name)
